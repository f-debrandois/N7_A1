...
package body Cache is
    ...
    function L_Interface (Cache : in out T_Cache ; ippaquet : in T_IP; politique : in T_Politique) return Unbounded_String is
    begin
        case politique is
            when FIFO => return Cache_FIFO.L_Interface(Cache, ippaquet);
            when LRU => return Cache_LRU.L_Interface(Cache, ippaquet);
            when LFU => return Cache_LFU.L_Interface(Cache, ippaquet);
    ...
    procedure Enregistrer(Cache: in out T_Cache;  dest: T_IP; masque: T_IP; interf: Unbounded_String; Politique: T_Politique) is
    begin
        case politique is
            when FIFO => Cache_FIFO.Enregistrer(Cache, dest, masque, interf);
            when LRU => Cache_LRU.Enregistrer(Cache, dest, masque, interf);
            when LFU => Cache_LFU.Enregistrer(Cache, dest, masque, interf);
            when others => null;
        end case;
    end Enregistrer;
end Cache;
