...
% 5.3 Filtrage
Xpb = filter(h_pb, 1, [x+bruit, zeros(1, (ordre - 1)/2)]);
Xph = filter(h_ph, 1, [x+bruit, zeros(1, (ordre - 1)/2)]);
...
xlabel("Fréquence (Hz)");
ylabel('Signal');
title("Densité spectrale de puissance du signal modulé et réponse en fréquences du filtre passe-bas" );
...
xlabel("Fréquence (Hz)");
ylabel('Signal');
title("Densité spectrale de puissance du signal modulé et réponse en fréquences du filtre passe-haut" );
legend('DSP', 'Réponse en fréquences');
