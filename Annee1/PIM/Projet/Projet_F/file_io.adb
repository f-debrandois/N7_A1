...
package body File_IO is
    ...
    procedure Afficher (Destination : in T_IP; Masque : in T_IP; Interf : in Unbounded_String )is
    begin
        Put_Line (Conversion_String(Destination) & " " & Conversion_String(Masque) & " " & Interf);
    end Afficher;

    -- Instancier la fonction generique Traiter par Afficher
    procedure AfficherRouteur is new Routeur.Pour_Chaque(Afficher);

    -- Traitement du cache lorsqu'on aura une ippaquet(destination)
    procedure Remplir_Cache (Cache : in out T_Cache; Tab : in T_Table; Interf : in Unbounded_String; IP_Paquet : in T_IP; Valeur_Initiale : in Valeur_Entree)is
    ...
    begin
        ...
        Enregistrer(Cache, Destination, Masque, Interf, Valeur_Initiale.Politique);
    end Remplir_Cache;
        ...
        loop
            ...
            if Ligne = "table" then
                ...
                if Est_Present(Cache, IP_Paquet, Valeur_Initiale.Politique) then
                    Interf := L_Interface (Cache, To_Ip(Ligne), Valeur_Initiale.Politique);
                else
                    Valeur_Initiale.Nombre_Defaut_Cache := Valeur_Initiale.Nombre_Defaut_Cache + 1;
                    Interf := L_Interface(Tab, To_Ip(Ligne));
                    Remplir_Cache (Cache, Tab, Interf, IP_Paquet, Valeur_Initiale);
                end if;
                Put (Sortie_File, Conversion_String(IP_Paquet) & " " & Interf);     --Écrire l'adresse paquet et son interface correspondante sur 2 postions sur le fichier Sortie
            end if;            
            exit when End_Of_File (Paquet_File);
