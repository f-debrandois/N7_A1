...
package Cache_LRU is

    Capacite : Integer := 2;

    -- Initialiser le cache.  La cache est vide.
    procedure Initialiser(Cache: out T_Cache; Capacite_Entree : in Integer) with
            Post => Est_Vide (Cache);

    -- Est-ce qu'une Cache est vide ?
    ...
    -- Enregistrer une adresse destination associée à une masque dans le cache.
    -- Si l'adresse est déjà présente dans le cache, son masque est changé.
